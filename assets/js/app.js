...
const state = {
  ...
  selectedName: null,
  meta: {},
  plotManifest: { target_plots: {}, global_plots: {} },
};
...
function chooseInitialPlanet(planets) {
  ...
  return planets.find((planet) => {
    ...
    return isPositive(planet.radius_earth)
      && isPositive(planet.mass_earth)
      && isPositive(science.transitDepthPpm)
      && isPositive(science.rvSemiAmplitude);
  }) ?? planets[0] ?? null;
}

function targetPlotPath(planet, key) {
  if (!planet || !state.plotManifest || !state.plotManifest.target_plots) return null;
  const target = state.plotManifest.target_plots[planet.name];
  return target?.[key] ?? null;
}


function globalPlotPath(key) {
  if (!state.plotManifest || !state.plotManifest.global_plots) return null;
  return state.plotManifest.global_plots[key] ?? null;
}

function renderGlobalPlot(container, key, altText) {
  const path = globalPlotPath(key);
  if (!path) return false;
  const safePath = escapeHtml(path);
  container.innerHTML = `
    <figure class="python-figure python-figure-global">
      <a class="plot-link" href="${safePath}" target="_blank" rel="noopener" aria-label="Open full-size figure: ${escapeHtml(altText)}">
        <img src="${safePath}" alt="${escapeHtml(altText)}" loading="lazy" decoding="async" />
      </a>
      <figcaption>
        <span>Python/Matplotlib SVG generated from the loaded JSON catalog.</span>
        <a class="plot-open-link" href="${safePath}" target="_blank" rel="noopener">Open full-size SVG</a>
      </figcaption>
    </figure>
  `;
  return true;
...
function renderGeneratedPlot(container, planet, key, altText) {
  const path = targetPlotPath(planet, key);
  if (!path) return false;
  const safePath = escapeHtml(path);
  container.innerHTML = `
    <figure class="python-figure">
      <a class="plot-link" href="${safePath}" target="_blank" rel="noopener" aria-label="Open full-size figure: ${escapeHtml(altText)}">
        <img src="${safePath}" alt="${escapeHtml(altText)}" loading="lazy" decoding="async" />
      </a>
      <figcaption>
        <span>Python/Matplotlib SVG generated during the data-refresh workflow.</span>
        <a class="plot-open-link" href="${safePath}" target="_blank" rel="noopener">Open full-size SVG</a>
      </figcaption>
    </figure>
  `;
...
function renderHzGraphic(planet, science) {
  if (renderGeneratedPlot(elements.hzGraphic, planet, 'habitable_zone', `Habitable-zone placement plot for ${planet.name}`)) return;
...
function renderTransitGraphic(planet, science) {
  if (renderGeneratedPlot(elements.transitGraphic, planet, 'transit_light_curve', `Transit light-curve model for ${planet.name}`)) return;
...
function renderRvGraphic(planet, science) {
  if (renderGeneratedPlot(elements.rvGraphic, planet, 'radial_velocity', `Radial-velocity model for ${planet.name}`)) return;
...
function renderMassRadiusGraphic(selectedPlanet, selectedScience) {
  if (renderGeneratedPlot(elements.massRadiusGraphic, selectedPlanet, 'mass_radius_context', `Mass-radius context plot highlighting ${selectedPlanet.name}`)) return;
...
function renderCaveats(science) {
  const baseNotes = [
    ...
    'The lab figures prefer static Python/Matplotlib SVGs generated during data refresh; browser SVG fallbacks are used only when a target-specific plot pack is unavailable.',
  ];
...
async function loadPlotManifest() {
  try {
    const response = await fetch(PLOT_MANIFEST_URL);
    if (!response.ok) return;
    state.plotManifest = await response.json();
  } catch (error) {
    state.plotManifest = { target_plots: {}, global_plots: {} };
    console.warn('Plot manifest unavailable; using browser-generated fallback plots.', error);
  }
}
...
async function loadData() {
  try {
    ...
    state.selectedName = chooseInitialPlanet(state.filtered)?.name ?? null;
