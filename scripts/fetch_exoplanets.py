...
(`pscomppars`), which provides one row per planet and is useful for broad
population visualizations and educational target triage.
...
# Keep these fields deliberately explicit. GitHub Pages serves a static JSON file,
# so the browser-side science lab benefits from stellar and orbital quantities
# beyond the minimal planet catalog columns.
FIELDS = [
    "pl_name",
    "hostname",
    "discoverymethod",
...
def fetch_json(url: str, timeout: int) -> list[dict[str, Any]]:
    request = urllib.request.Request(url, headers={"User-Agent": "open-exoplanet-explorer/2.0"})
...
def linear_luminosity_from_log(log_luminosity: float | int | None) -> float | None:
    if log_luminosity is None:
        return None
    luminosity = 10 ** float(log_luminosity)
    if not math.isfinite(luminosity):
        return None
    return round(luminosity, 8)


def normalize_record(record: dict[str, Any]) -> dict[str, Any]:
    st_lum_log = number_or_none(record.get("st_lum"))
    return {
        ...
        "eccentricity": number_or_none(record.get("pl_orbeccen")),
        "inclination_deg": number_or_none(record.get("pl_orbincl")),
        "equilibrium_temp_k": number_or_none(record.get("pl_eqt")),
        "insolation_earth": number_or_none(record.get("pl_insol")),
        "density_g_cm3": number_or_none(record.get("pl_dens")),
        "distance_pc": number_or_none(record.get("sy_dist")),
        "vmag": number_or_none(record.get("sy_vmag")),
        "kmag": number_or_none(record.get("sy_kmag")),
        "stellar_temp_k": number_or_none(record.get("st_teff")),
        "spectral_type": text_or_unknown(record.get("st_spectype")),
        "stellar_radius_solar": number_or_none(record.get("st_rad")),
        "stellar_mass_solar": number_or_none(record.get("st_mass")),
        # NASA's st_lum is log10(L/Lsun); expose both log and linear values for the browser.
        "stellar_luminosity_log_solar": st_lum_log,
        "stellar_luminosity_solar": linear_luminosity_from_log(st_lum_log),
        "tag": "Archive record",
        "description": "",
    }
...
def main() -> int:
    ...
    payload = {
        "meta": {
            ...
            "notes": (
                "Generated by scripts/fetch_exoplanets.py for static GitHub Pages publishing. "
                "The browser derives educational science quantities from these archive fields. "
                "Use the NASA Exoplanet Archive directly for publication-grade data checks."
            ),
        },
        "planets": planets,
    }
